...
Use ARROWS or WASD keys for control.
   ...
   (`            : next sensor)
    ...
    Backspace    : start new recording of expert demonstration

    R            : toggle recording demonstrations to disk
    ...
    Saving expert demonstration videos starts automatically.
    Press backspace to start a new recording
"""
...
###############################################################################
from cv2 import imread, imwrite, resize, INTER_CUBIC, cvtColor, COLOR_BGR2RGB

class Observation():
    """Stores the driving information of the image
    """
    ...
    out_count = 0
    # Save every hop:th image to disk
    hop = 6
###############################################################################
...
###############################################################################
                    # Increase the count of demosntration folders
                    Observation.out_count += 1
                    print('Current demonstration folder: ', out_count)
###############################################################################
                    world.restart()
                elif event.key == K_F1:
...
###############################################################################
            Observation.image_throttle = c.throttle
            Observation.image_steer = c.steer
            Observation.image_brake = c.brake
###############################################################################
...
###############################################################################
        ...
        if self._recording:
            # Set hop for recording
            if image.frame_number % Observation.hop != 0:
                return
            # Set filename
            directory_filename = "images/out" + str(Observation.out_count) + "/"
            ...
            # Save resized image
            imwrite(image_filename+".png",cvtColor(resize(array_copy, \
                (299,299), interpolation=INTER_CUBIC), COLOR_BGR2RGB))
            # image.save_to_disk(image_filename)
            # Save driving info
            with open(image_filename+".txt", "w+") as text_file:
                print(str(Observation.image_throttle)+";"\
                    +str(Observation.image_steer)+";"\
                        +str(Observation.image_brake), file=text_file)
...
##############################################################################
    ...
    for out_folder in out_folders:
        Observation.out_count
        Observation.out_count += 1

    print("images subfolders count: ", Observation.out_count)
##############################################################################
